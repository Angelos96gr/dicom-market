...
function Login() {
    ...
    useEffect(() => {
        ...
        console.log(logSubmissionStatus)
        alert("congratuatlations")

        const payload = { "email": email, "password": pwd }
        if (email) {
            console.log("Sending Axios request")
            axios.post(loginUrl, payload).then((resp) => {
                alert("congratuatlations from within")
        ...
        <div className="flex pt-32 justify-center">
            <div>
                <form className={formStyle}>
                    ...
                    <button className={buttonClass} name="login" type="submit" onClick={readCredentials}>Login</button>
